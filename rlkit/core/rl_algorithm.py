...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    def __init__(
        ...
        self.save_enc_replay_buffer = save_enc_replay_buffer
        self.save_algorithm = save_algorithm
        self.save_environment = save_environment

        self.resample_in_traj = resample_in_traj
        ...
        assert self.batch_size % self.traj_batch_size == 0
        ...
        '''!
        extra argument, number of mini trajectory
        '''
        ...
        self._n_env_steps_total = _n_env_steps_total
        self._n_train_steps_total = _n_train_steps_total
        self._n_rollouts_total = _n_rollouts_total
    ...
    def train(self):
        ...
        for it_ in gt.timed_for(
                range(self.start_epoch, self.num_iterations),
                save_itrs=True,
        ):
            ...
            if it_ == 0:
                ...
                for idx in tqdm(self.train_tasks, disable=not self.debug):
                    self.task_idx = idx
            ...
            for idx in tqdm(random.sample(range(len(self.train_tasks)), self.num_tasks_sample), disable=not self.debug):
                self.task_idx = idx
                self.env.reset_task(idx)
                ...
                if self.num_extra_rl_steps_posterior > 0: #! don't know why this step matters
                    self.collect_data(self.num_extra_rl_steps_posterior, 1, self.update_post_train, add_to_enc_buffer=False)
            # Sample train tasks and compute gradient updates on parameters.
            for train_step in tqdm(range(self.num_train_steps_per_itr), disable=not self.debug):
            ...
            # eval
            self._try_to_eval(it_, eval_all=False)
            gt.stamp('eval')
            self._end_epoch()
    ...
    def collect_data(self, num_samples, resample_z_rate, update_posterior_rate, add_to_enc_buffer=True):
        ...
        while num_transitions < num_samples:
            paths, n_samples = self.sampler.obtain_samples(max_samples=num_samples - num_transitions,
                                                                ...
                                                                glob=self.glob,
                                                                resample_in_traj=self.resample_in_traj)
    ...
    def get_extra_data_to_save(self, epoch):
        ...
        if self.save_enc_replay_buffer:
    ...
    def _do_eval(self, indices, epoch, animated=False):
        ...
        for idx in tqdm(indices, disable=not self.debug):
            all_rets = []
    ...
    def evaluate(self, epoch, eval_all=False, eval_train_offline=True, animated=False):
        # import pdb
        # pdb.set_trace()
        ...
        train_final_returns, train_online_returns = self._do_eval(indices, epoch, animated)
        # print (self.agent.n_infer, self.agent.n_global, self.agent.n_local)
        eval_util.dprint('train online returns')
        eval_util.dprint(train_online_returns)
        ### test tasks
        eval_util.dprint('evaluating on {} test tasks'.format(len(self.eval_tasks)))
        test_final_returns, test_online_returns = self._do_eval(self.eval_tasks, epoch, animated)
        # print (self.agent.n_infer, self.agent.n_global, self.agent.n_local)
        ...
        if hasattr(self.env, "log_diagnostics") and eval_train_offline:
        ...
        logger.save_extra_data({'online-train': avg_train_online_return,
                                'online-test': avg_test_online_return,
                                }, path='data-epoch{}'.format(epoch))
