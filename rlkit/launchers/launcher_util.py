...
def create_log_dir(exp_prefix, exp_id=None, seed=0, base_log_dir=None, config_log_dir=None):
    """
    ...
    if config_log_dir != None:
        log_dir = osp.join(base_log_dir, exp_prefix.replace("_", "-"), config_log_dir, exp_name)
    else:
...
def setup_logger(
    ...
    if first_time:
        log_dir = create_log_dir(exp_prefix, exp_id=exp_id, seed=seed,
                                 base_log_dir=base_log_dir, config_log_dir=config_log_dir)
    ...
    # sys.stderr = open(osp.join(log_dir, "debug.err"), "w")
    return log_dir
